...
import {
    IAllKeysQuery,
    IAllKeysQueryResponse,
    IGetKeyQuery,
    IGetKeyQueryResponse,
    IKernelKeyQuery,
} from "./types";
...
export const getClient = cache(async (chainId: string) => {
    try {
        ...
        const client = await CosmWasmClient.connect(
            config.data.chainConfigs.config.chainUrl
        );
        return client;
    } catch (err) {
        console.log("Chain config not found for ", chainId);
        console.log(
            JSON.stringify((err as ApolloError).graphQLErrors, undefined, 2)
        );
        throw err;
    }
});
...
export const getEmbeddableAddress = cache(async (client: CosmWasmClient) => {
    ...
    const key: string = await client.queryContractSmart(
        config.data.chainConfigs.config.kernelAddress,
        query
    );
    return key;
});
...
export const getConfig = cache(async (client: CosmWasmClient, key: string) => {
    ...
    const rawConfig: IGetKeyQueryResponse = await client.queryContractSmart(
        db,
        query
    );
...
export const getAllApps = cache(async (client: CosmWasmClient) => {
    ...
    const db = await getEmbeddableAddress(client);
    console.log(db, chainId, "DB");
    const keys: IAllKeysQueryResponse = await client.queryContractSmart(
        db,
        query
    );
    return keys;
